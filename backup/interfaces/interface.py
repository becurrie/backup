...
class Interface(ABC):
    ...
    def __init__(self, config):
        ...
        Raises:
            ValidationError: If the `config` instance does not adhere to the model
...
class BackupInterface(Interface):
    ...
    def archive(self, directory, src):
        ...
        Args:
            directory: The directory configuration object for the backup interface.
...
class VaultInterface(Interface):
    ...
    def load(self):
        ...
        This method should be implemented by subclasses to define the specific
        behavior of loading secrets from the vault. The implementation should
        handle the process of loading secrets from the configured vault service.

        This method might store the secrets in the environment for use by other
        components or interfaces, or it might return the secrets to the caller
